...
async def crear_fletero(
    ...
    """Crea un nuevo fletero. Owner y Admin. El email debe ser real para poder recuperar la contraseña."""
    from app.auth import get_password_hash
    from app.services.queries.fletero_service import get_fleteros_con_estado
    ...
    if existing:
        return templates.TemplateResponse(
            ...
            {"user": current_user, "fletero_edit": None,
             "error": f"Ya existe un usuario con el email {login_email}."},
        )
...
async def editar_fletero_guardar(
    ...
    """Guarda cambios en un fletero. Permite cambiar email y otros datos."""
    from app.services.queries.fletero_service import get_fleteros_con_estado
    ...
    if new_email != fletero.email:
        existing = await usuario_repo.get_by_email(db, new_email, current_user.empresa_id)
